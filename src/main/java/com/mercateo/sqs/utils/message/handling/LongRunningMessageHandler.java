...
public class LongRunningMessageHandler<I, O> {
  ...
  LongRunningMessageHandler(
      @NonNull ScheduledExecutorService timeoutExtensionExecutor,
    ...
    if (timeUntilVisibilityTimeoutExtension.isZero()
        || timeUntilVisibilityTimeoutExtension.isNegative()) {
    ...
    if (queue
            ...
            .minusSeconds(5)
            .compareTo(timeUntilVisibilityTimeoutExtension)
        < 0) {
      throw new IllegalStateException(
          "The extension interval of "
              + timeUntilVisibilityTimeoutExtension.getSeconds()
              + " is too close to the VisibilityTimeout of "
              + queue.getDefaultVisibilityTimeout().getSeconds()
              + " seconds of the queue, has to be at least 5 seconds less.");
    }
  }

  /**
   * Submits a task for the processing of the message into the internal executor. Schedules a
   * timeoutExtender that takes care of extending the visibility timeout until and during message
   * processing.
   *
   * <p>Returns iff there is at least one free slot in the internal executor i.e. that new messages
   * can be consumed. That way we guarantee that we can handle an incoming maxNumberOfMessages on
   * the next iteration. Returning from this method does <b>not</b> mean the message has already
   * been processed, it simply means that it is in processing.
   *
   * <p>This method should only be called from a single thread, from a single SqsListener and only
   * once per message.
   *
   * <p>The SimpleMessageListenerContainer dispatches one task per incoming message to an internal
   * ThreadPoolExecutor and waits for all the tasks to finish before polling from SQS again. That
   * means we can block each task / thread from returning until a free worker is available without
   * interfering with the dispatching of other message tasks.
   *
  ...
  private void scheduleNewMessageTask(
      @NonNull Message<I> message, ScheduledFuture<?> visibilityTimeoutExtender) {
    MessageHandlingRunnable<I, O> messageTask =
        messageHandlingRunnableFactory.get(
            worker,
            message,
